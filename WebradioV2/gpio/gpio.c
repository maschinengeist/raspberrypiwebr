...
/***********************************************************************
 */
/*! \fn RESULT gpio_Direction(INT_U8 u08GpioPin, INT_U8 u08Direction)
 *
 *   \brief Init an gpio
 *
 *   \param u08GpioPin --> the gpio nummber to init
...
RESULT gpio_Direction(INT_U8 u08GpioPin, INT_U8 u08Direction)
{
	...
	else
	{
		...
		if(u08Direction == GPIO_DIRECTION_IN) fwrite("in", 1, strlen("in"), fd_gpio);
		else fwrite("out", 1, strlen("out"), fd_gpio);
